  ...
  try {
    ...
    if (state == 'draft') {
      const blogs = await blogModel
        .find({ userId: userId, state: 'draft' })
        .limit(limit)
        .skip(skip);
    ...
    if (state == 'published') {
      const blogs = await blogModel
        .find({ userId: userId, state: 'published' })
        .limit(limit)
        .skip(skip);
      return res.status(200).json(blogs);
    }
...
async function updateBlogState(req, res, next) {
  try {
    ...
    if (user) {
      const updateState = await blogModel.findByIdAndUpdate(
        { _id: blogId },
        { state: state },
        { new: true }
      );
